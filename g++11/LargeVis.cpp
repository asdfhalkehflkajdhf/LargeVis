...
//c++11 随机数生成说明　http://blog.jobbole.com/105780/
//http://en.cppreference.com/w/cpp/numeric/random/uniform_real_distribution
//这个不能写在类里面作为一个变量　，在函数，全局变量，在成员函数里，都是可以的。
std::default_random_engine g_random_e;
std::uniform_real_distribution<double> g_random_urd_d(0.0, 1.0);
...
void LargeVis::run_annoy()
{
	...
	std::thread pt[n_threads];
	for (int j = 0; j < n_threads; ++j) pt[j]=std::thread(LargeVis::annoy_thread_caller, new arg_struct(this, j));
...
void LargeVis::run_propagation()
{
	for (int i = 0; i < n_propagations; ++i)
	{
		...
		std::thread pt[n_threads];
		for (int j = 0; j < n_threads; ++j) pt[j]=std::thread(LargeVis::propagation_thread_caller, new arg_struct(this, j));
		for (int j = 0; j < n_threads; ++j) pt[j].join();
...
void LargeVis::compute_similarity()
{
	...
	std::thread pt[n_threads];
	for (int j = 0; j < n_threads; ++j) pt[j]=std::thread(LargeVis::compute_similarity_thread_caller, new arg_struct(this, j));
	for (int j = 0; j < n_threads; ++j) pt[j].join();

	for (int j = 0; j < n_threads; ++j) pt[j]=std::thread(LargeVis::search_reverse_thread_caller, new arg_struct(this, j));
	for (int j = 0; j < n_threads; ++j) pt[j].join();
...
void LargeVis::test_accuracy()
{
	...
	{
		x = floor(g_random_urd_d(g_random_e) * (n_vertices - 0.1));
...
void LargeVis::visualize_thread(int id)
{
	...
	while (1)
	{
		...
		p = sample_an_edge(g_random_urd_d(g_random_e), g_random_urd_d(g_random_e));
		x = edge_from[p];
		y = edge_to[p];
		lx = x * out_dim;
		...
		for (i = 0; i < n_negatives + 1; ++i)
		{
			if (i > 0)
			{
				y = neg_table[(unsigned long long)floor(g_random_urd_d(g_random_e) * (neg_size - 0.1))];
...
void LargeVis::visualize()
{
	...
	for (i = 0; i < n_vertices * out_dim; ++i) vis[i] = (g_random_urd_d(g_random_e) - 0.5) / out_dim * 0.0001;
	init_neg_table();
	init_alias_table();
	edge_count_actual = 0;
	
	std::thread pt[n_threads];
	for (int j = 0; j < n_threads; ++j) pt[j]=std::thread(LargeVis::visualize_thread_caller, new arg_struct(this, j));
	for (int j = 0; j < n_threads; ++j) pt[j].join();
